...
void GlobalHistogram::set(std::map<int, TH2F*>& histB, std::map<int, TH2F*>& histNB, bool doAverage)
{
  for(auto& ih : histB) {
    ...
    float xMin[2] = {xB0 - bboxB.width() / 2, xNB0 - bboxNB.width() / 2};
    float xMax[2] = {xB0 + bboxB.width() / 2, xNB0 + bboxNB.width() / 2};
    float yMin[2] = {yB0 + bboxB.ymin(), yNB0 + bboxNB.ymin()};
    float yMax[2] = {yB0 + bboxB.ymax(), yNB0 + bboxNB.ymax()};
